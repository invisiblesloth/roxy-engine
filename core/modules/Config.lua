...
local peekProxy = setmetatable({}, {
  __index = function(_, key) return activeConfig[key] end,
  __newindex = function()
    Log.error("Config table is read-only. Use Config.setConfig() to modify.", 2)
  end,
  __pairs = function() return pairs(activeConfig) end
})
...
-- This is a convenience method that combines setConfig with change notification.
